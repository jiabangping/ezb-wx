...
function secondSubmit() {
	$("#dataSubmit").show();
	var secondSubUrl = "doctor/overseaCompleteInfo";
	 $.ajax({
		url : secondSubUrl,
		type : 'post',
		cache : false,
		dataType : 'json',
		 ...
		 'doctorId':$("#doctorId").val(),
       	  'overSea' : $("#overseaHidden").val(),
       	  'sex' : $('input:radio[name=Sex]:checked').val(),
   	      'doctorName' : $("#DoctorName").val(),
   	      'email':$("#email").val(),
   	     // 'chargingStandard':$("#chargingStandard2").val(),
   	      'phoneNum':$("#PhoneNum").val(),
   	      'idCard':$("#IDCard").val(),
		...
		success : function(data) {
			...
			if(data.result == 'success' ) {
				$("#signSuccessBtn").text("确定");
				$("#signSuccessModalContent").text("您的资料提交成功,我们会及时通知您审核结果。");//这里用得是signUpPage内的signSuccessModal模态框不是公共的
				$('#signSuccessModal').modal({backdrop: 'static', keyboard: false});
				$("#signSuccessModal").modal("show");
			}else if(data.result == 'fail') {
...
//	uploadProcessTimer = window.setInterval(getFileUploadProcess, 20);
	...
	$.ajaxFileUpload ({
		...
		success: function (data, status) {
			...
			$("#selfPhotoUrlHidden").val(data.url+"?timestamp="+(new Date().getTime()));
			$("#photo").attr("src",data.url+"?timestamp="+(new Date().getTime()));
			$("#photo").show();
			//清除定时器
			...
			if(overseaEmail != "") {
			 ...
			 if(document.getElementById("chargingStandard") == null) {
			 	...
			 	"<div class='form-group' id='chargingStandard'>"+
		            "<label class='control-label' for='inputError1'>收费标准(元/次)</label>"+
...
$(function() {	
	if(($("#overseaHidden").val() * 1) == 0) {
		$("input[name=Oversea][value=0]").attr({"checked":"true","disabled":"true"});
	}else {
		$("input[name=Oversea][value=1]").attr({"checked":"true","disabled":"true"});
	}
	$("input[name=Oversea]").attr({"disabled":"true"});
	
	if(($("#sexHidden").val() * 1) == 0) {
		$("input[name=Sex][value=0]").attr({"checked":"true"});
	}else {
		$("input[name=Sex][value=1]").attr({"checked":"true"});
	}
	$("input[name=Sex]").attr({"disabled":"true"});
	$("#photo").attr("src",$("#selfPhotoUrlHidden").val()+"?t="+(new Date().getTime()));
